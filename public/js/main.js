...
async function fetchData() {
  ...
  div.classList.add("message", "mb-4"); // container with spacing

  const p = document.createElement("p");
  p.classList.add("meta", "font-bold", "text-indigo-500"); // username
  p.innerHTML = `${message.username} <span class="ml-2 text-sm text-gray-500">${message.time}</span>`;
  div.appendChild(p);

  const para = document.createElement("p");
  para.classList.add("text", "text-black", "mt-1"); // message text explicitly black
